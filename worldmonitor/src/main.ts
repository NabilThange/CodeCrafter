...
import './styles/vane-navigation.css';
...
if (urlParams.get('settings') === '1') {
  ...
  app
    ...
    .then(() => {
      ...
      // Add Vane navigation sidebar after app initialization
      import('./components/VaneNavigation').then(({ VaneNavigation }) => {
        const vaneNav = new VaneNavigation(document.body);
        vaneNav.render();
      }).catch(console.error);
    })
    .catch(console.error);
